...
export default function ser({services}){
    return (
        <>

            { services.length>=1? ( <div className='min-h-screen'>
                                          <h1 className=' text-center text-xl font-bold w-5/6 my-8 mx-auto'>Services</h1>
                                          <div className='card-container mb-3 flex justify-between flex-wrap'>
                                            <Product_Card productsData={services} />
                                          </div>
                                       </div>)
                                    :
...
export async function getStaticProps() {
    ...
    const services = await client.fetch(`*[_type == "product" && category == 'service']`);
    return {
      props: {
        services
      }
