...
export default function({popular}){
    return (
      <>
      { popular.length>=1 ? ( <div className='min-h-screen'>
                                    <h1 className=' text-center text-xl font-bold w-5/6 my-8 mx-auto'>Popular Deals</h1>
                                    <div className='card-container mb-3 flex justify-between flex-wrap'>
                                      <Product_Card productsData={popular} />
                                    </div>
                                 </div>)
                              :
...
export async function getStaticProps() {
    ...
    const popular = await client.fetch(`*[_type == "product" && ('popular' in tags)]`);
    return {
      props: {
        popular
      }
